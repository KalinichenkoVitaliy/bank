...
export class CustomSelect {
  ...
  _update(option) {
    option = option.closest(SELECTOR_OPTIONS);
  ...
  _reset() {
    ...
    this._elToggle.textContent = selectPlaceHolder;
  ...
  set value(value) {
    let isExists = false;
    this._elRoot.querySelectorAll(SELECTOR_OPTIONS).forEach((option) => {
  ...
  set selectedIndex(index) {
    const option = this._elRoot.querySelector(`${SELECTOR_OPTIONS}[data-index="${index}"]`);
    if (option) {
      return this._update(option);
    }
    return this._reset();
...
CustomSelect.template = (params) => {
  ...
  if (params['placeHolder']) selectPlaceHolder = params['placeHolder'];
  let selectedContent = selectPlaceHolder;
